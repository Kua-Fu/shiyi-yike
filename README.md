...
| 诗库搜索 | 内置 5334 篇诗词和 928 位作者资料，支持忽略标点的全文搜索、结果高亮及可检索作者筛选 |
| 阅读分享 | 提供简繁切换、专注模式、收藏、阅读统计、诗篇直达链接与高清分享图，扩展运行时无需联网 |

官网构建会为 100 篇深度精读生成独立 HTML、结构化数据与 sitemap 条目；网页和扩展发布包使用按诗库字符集生成的 WOFF2 行书子集。

## 维护与构建

- `npm test`：运行数据、纯逻辑、网页产物以及真实 Chrome 无障碍回归测试。
- `npm run build:data`：在同文件系统临时副本内生成并校验完整诗库，成功后整体替换；远程语料由 `data/sources/upstream-lock.json` 固定 commit 与 SHA-256。
- `npm run build:font`：按当前诗库字符集重建 WOFF2 字体子集，需先安装 `requirements-fonts.txt`。
- `npm run build:web`：生成官网、在线阅读器、100 篇静态精读页和 sitemap。
